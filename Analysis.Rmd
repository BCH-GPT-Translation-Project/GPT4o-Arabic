...
p <- translations |>
	filter(label == "GPT") |> 
	ggplot(aes(x = as.numeric(factor(unit)) + as.numeric(factor(linguist)) * 0.2 - 0.52, y = preference, shape = linguist)) +
		scale_x_continuous(breaks = unique(as.numeric(factor(translations$unit))),
...
p
ggsave("preference_distribution.png", plot = p, width = 12, height = 3, dpi = 1200, bg = "white")
```
